...
Seam: the `Store` trait plus `oxe-core::conformance` (W0-04, hardened in W6-01), which is
...
Postgres `Store` impl (former W6-02): `crates/oxe-store-postgres` via `sqlx`
...
`oxe-cli`, `oxe store migrate` for both backends, and a CI job with a Postgres service
...
(`[auth] admin_tokens = ["${env:OXE_ADMIN_TOKEN}"]`) required for `DELETE /api/cache*`,
...
Compose (oxe + Postgres + optional OTel collector + Grafana dashboards JSON for the W1-09

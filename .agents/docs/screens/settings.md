...
|   API key    [ ${env:CAUCE_AI_API_KEY}                ]           |
|              CAUCE_AI_API_KEY is set                              |
...
- Templates are never resolved: an `${env:NAME}` value renders verbatim in
  ...
  the `${env:CAUCE_AI_API_KEY}` template survives a save round trip
...
- AI answers section is visibly greyed (reduced opacity, `lands in wave 4`
  ...
  `CAUCE_AI_API_KEY is set` or `is not set` when the value is an env

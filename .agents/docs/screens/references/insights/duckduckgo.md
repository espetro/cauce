...
general rule in `patterns-*.md`. This analysis matters doubly for oxe because oxe's
search backend IS DuckDuckGo: anything DDG does at the pipeline level (snippets,
favicons, result anatomy, rate-limit surfaces) shapes what oxe's UI can honestly
display.
...
with one underlined word. Compare oxe's landing which carries a wordmark, tagline,
...
oxe's `history.md` this is the closest precedent for a local-only, privacy-framed
...
`patterns-states.md`'s calm rate-limit treatment; oxe's "Rate limit reached,
...
For oxe this validates the rubric's anti-pattern list (7.6): motion budget goes to
...
panel, no numbered-marker-to-card highlighting like oxe's rubric 3.5 requires. For
oxe this is a gap DDG leaves open: oxe's inline `[n]` markers linked to source
cards (rubric 3.5, 3.6) are richer than anything DDG ships, and since oxe's backend
returns the actual DDG result set, oxe can wire citations to real fetched documents
in a way Duck.ai's proxy architecture does not surface.

### 4.3 Backend-relevant notes for a DDG-backed pipeline (oxe-specific)

- Snippets: DDG result snippets are plain text with bolded query terms (visible in
  the dark SERP shot: "Sample" bolded inside the snippet). oxe receives this and
...
- Favicons: DDG SERP shows favicons inline with the https://domain line at ~16px.
  oxe must handle the same favicon-availability problems (DDG serves them via its
...
- Sitelinks: DDG augments the first result with a multi-column sitelinks block
  (About / Blog / Funk Soul / Ambient... in the shot). oxe's DDG backend does not
  return sitelinks; oxe's flat result list is honest to its data, do not fake it.
- Rate limiting: DDG intermittently throttles automated/anonymous queries (CAPTCHA
  or 403 on the HTML endpoints). oxe's cache exists partly to absorb this; the UX
  implication is that oxe's `cached · <age>` badge and "served from cache" honesty
  ...
  `patterns-states.md` yet and is worth adding as an oxe-specific finding.
- No result-count or timing metadata comes back reliably from DDG; oxe's meta line
...
- Content column: the classic SERP column is roughly 652px, left-anchored within
  ...
  oxe's ~600-652px search / ~680px AI column convention (rubric 5.5).
...
- Accent: DDG orange (#de5833 family) is used almost exclusively for the brand mark
  ...
  saturated result titles in a soft periwinkle. oxe's dark mode should target the
...
1. Keep the citation bar higher than DDG's. Search Assist's end-of-paragraph domain
   ...
   major AI search surfaces; oxe's per-claim `[n]` markers linked to source cards
   are a real differentiator, and the DDG backend supplies real URLs to back them.
2. Add a "stale cache on throttle" state. Because oxe's backend is DDG, intermittent
   ...
   `patterns-states.md` as an oxe-specific state.
3. Copy DDG's privacy-chrome placement, not its sparseness. The persistent
   ...
   copy on landing are cheap, effective patterns oxe can mirror (e.g. a "served
...
5. Mirror the always-visible model control. DDG keeps the current model as a
   ...
   menu; oxe's model selector should show the active model inline per turn and

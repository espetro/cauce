        ...
        if tool_calls:
            ...
            chunks.append(
                SimpleNamespace(
                    choices=[
                        SimpleNamespace(
                            delta=SimpleNamespace(content=None, tool_calls=tc_parts)
                        )
                    ]
                )
            )
...
def test_web_search_tool_wraps_do_search(tmp_path):
    ...
    class StubBackend:
        name = "stub"
        def search(self, req):
            return {"requestId": "r", "searchType": "auto", "results": [
                {"title": "T", "url": "https://example.com/s", "text": "b",
                 "highlights": [], "highlightScores": []}], "costDollars": {"total": 0.0}}
...
def test_answer_cache_hit_single_done_event(tmp_path):
    ...
    final = {"type": "done", "answer": "Hi", "related_questions": [],
             "confidence": 9, "model": "openai:gpt-test", "cached": False}

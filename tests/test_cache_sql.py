...
def test_set_get_roundtrip(tmp_path: Path) -> None:
    ...
    val: JSONDict = {"results": [1, 2], "_q": "hello"}
    c.set("h1", val, ttl=60)
    assert c.get("h1") == val
    c.close()
...
def test_hits_increment(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
...
def test_list_rows_filters(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
...
def test_clicks(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
    c.set("h1", {"results": [], "_q": "python tips"}, ttl=60)
...
def test_click_stats(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
...
def test_search_log(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
    c.log_search("python tips", "h1", "mcp", SearchLogMeta(result_count=3))
    c.log_search("golang", "h2", "http", SearchLogMeta(duration_ms=None))
...
def test_lookup_query_text(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
    c.set("h1", {"results": [], "_q": "python tips"}, ttl=60)
...
def test_prunes(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
    c.record_click("h1", "r1", "https://x.com", "X")
...
def test_delete_clicks_scopes(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")
    c.record_click("h1", "r1", "https://x.com", "X")
...
def test_fuzzy_query_ids_typo(tmp_path: Path) -> None:
    c = TTLCache(tmp_path / "c.db")

...
use oxe_core::{CacheKey, Store, StoreTuning};
...
async fn fts_rowid_in_sync_for_empty_results() {
    let (store, dir) = open();
    let path = dir.path().join("oxe.db");

    let empty_key = CacheKey::from(&oxe_core::conformance::request("fts empty"));
    let full_key = CacheKey::from(&oxe_core::conformance::request("fts full"));
    store
        .put(
            &empty_key,
            &oxe_core::conformance::response("fts empty", &[]),
    ...
    store
        .put(
            &full_key,
            &oxe_core::conformance::response("fts full", &[("T", "https://t.example.com/", "s")]),
            Duration::from_secs(3600),
        )
        .await
        .unwrap();
...
async fn reopen_is_idempotent() {
    ...
    let path = dir.path().join("oxe.db");
    let key = CacheKey::from(&oxe_core::conformance::request("reopen"));
    let resp = oxe_core::conformance::response("reopen", &[("R", "https://r.example.com/", "r")]);

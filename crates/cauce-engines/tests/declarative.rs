...
use cauce_core::config::{Config, EngineEntry, EngineKind, EnvMap};
use cauce_core::http::HttpClient;
use cauce_core::{ClientKind, Engine, EngineError, SafeSearch, SearchRequest, Tier};
...
request:
  ...
  headers:
    ...
    X-Key: "${env:CAUCE_TEST_SPEC_KEY:-no-key}"
  timeout_ms: 5000
parse:
  kind: html
  results: "div.result"
...
fn build_engines_auto_registers_unconfigured_specs() {
    // Requires CAUCE_ENGINES unset in the test process (the pin suppresses
    // auto-registration); CI and `mise run test` never set it.
    if std::env::var("CAUCE_ENGINES").is_ok_and(|v| !v.trim().is_empty()) {
        return;
    }
    let tmp = tempfile::tempdir().unwrap();

//! Acceptance tests for W0-06: `replay` engine and `cauce record` core.
//!
...
use cauce_engines::cassette::{Cassette, cassette_key, cassette_path};
use cauce_engines::{Replay, ReplayOpts, record};
...
#[test]
fn from_env_reads_cauce_replay_vars() {
    // Safety: nextest runs each test in its own process; these names are not
    // read by any other test in this binary.
    unsafe {
        std::env::set_var("CAUCE_REPLAY_LATENCY_MS", "42");
        std::env::set_var("CAUCE_REPLAY_FAIL_EVERY", "3");
        std::env::set_var("CAUCE_REPLAY_BLOCKED", "1");
        std::env::set_var("CAUCE_REPLAY_EMPTY", "true");
        std::env::set_var("CAUCE_REPLAY_PAGE_LIMIT", "4");
    }

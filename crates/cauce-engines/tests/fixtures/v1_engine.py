...
Flags:
  --bare-rejection   the version rejection line is out of contract:
                     `{"error":"unsupported protocol version: <v>"}` with no
                     `v` field at all, like a strict third-party child. The
                     parent's substring fallback must still downgrade.
"""
import argparse
import json
import os
...
def main() -> None:
    ...
    ap.add_argument("--bare-rejection", action="store_true")
    opts = ap.parse_args()
    pid = os.getpid()
    for line in sys.stdin:
        ...
        try:
            ...
            v = req.get("v")
            if v != 1:
                if opts.bare_rejection:
                    resp = {"error": f"unsupported protocol version: {v!r}"}
                else:
                    raise ValueError(f"unsupported protocol version: {v!r}")
            else:
                # v1 children see the settled fields only; anything extra
                # would be ignored here, but the parent must omit v2 fields
                # entirely.

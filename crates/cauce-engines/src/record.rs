...
//! (see [`crate::cassette`]). Backs `cauce record --engine <id> --query <q>`.
//!
...
use cauce_core::{ClientKind, Engine, EngineError, SafeSearch, SearchRequest};
...
///
/// Engine-agnostic: works for any `Engine` impl, so `cauce record` only maps

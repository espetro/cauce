//! `cauce engine test` fixture machinery (W1-02).
//!
...
use cauce_core::{
    ClientKind, EngineError, SafeSearch, SearchRequest, SearchResult, normalize_query,
};
...
/// Write a fixture pair under `<fixtures_root>/<id>/` (`cauce engine test
...
/// Load+compile a spec for `cauce engine test`: a literal file path first,
/// then the `$config_dir/engines/` + embedded lookup by name (so
/// `cauce engine test bing` works once the spec ships).

...
export function selectText(el, doc = document) {
...
export function copyNow(text, el, nav = navigator, doc = document) {
...
export function initRequestIdCopy(doc = document) {
  const el = doc.getElementById("page-request-id");
  if (!el) return;
  el.addEventListener("click", () => copyNow(el.textContent, el, navigator, doc));
}

/** `/trace/{id}`: copy button mirrors the traced request id. */
export function initTracedCopy(doc = document) {
  const btn = doc.getElementById("copy-traced");
  const el = doc.getElementById("traced-id");
  if (!btn || !el) return;
  btn.addEventListener("click", () => copyNow(el.textContent, el, navigator, doc));
}
 ...
 */
export function initHistoryCopy(doc = document) {
  if (!doc.body || doc.body.dataset.page !== "history") return;
  const copiedLabel = doc.body.dataset.copiedLabel || "";
  doc.addEventListener("click", async (e) => {
    const rid = e.target.closest ? e.target.closest(".request-id") : null;
    if (rid) {
      try {
    ...
    const a = e.target.closest ? e.target.closest("a.copy-json") : null;
    if (!a) return;
    e.preventDefault();
    const code = a.closest("td").querySelector(".copy-fallback");
    try {
...
export function initHashDetails(win = window, doc = document) {

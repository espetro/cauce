...
/// Wave-2 rows mounted so far. `/opensearch.xml` (W2-11) is a
/// `requires: "ui"` row: it mounts only when the `ui` cargo feature is
/// compiled in and `--headless` is not passed.
const EXPECTED_WAVE2_MOUNTED_UI: &[(&str, &str)] = &[("GET", "/opensearch.xml")];
...
fn mounted_routes_match_declaration() {
    ...
    if cfg!(feature = "ui") {
        expected.extend(
            EXPECTED_WAVE0_UI
                .iter()
                .chain(EXPECTED_WAVE2_MOUNTED_UI)
                .map(|(m, p)| (m.to_string(), p.to_string())),
        );
    }
    if cfg!(feature = "mcp") {
...
async fn headless_drops_ui_routes_keeps_api() {
    ...
    let headless: BTreeSet<(String, String)> = mounted_routes(&state, &RouterOptions::headless())
        .map(|r| (r.method.to_string(), r.path.to_string()))
        .collect();
    for ui_row in EXPECTED_WAVE0_UI.iter().chain(EXPECTED_WAVE2_MOUNTED_UI) {
        assert!(
    ...
    for uri in ["/", "/search?q=x", "/opensearch.xml"] {

...
//! `cauce_core::metrics`; this handle only bridges the async `Store` into the
//! `cauce_cache_entries` gauge before a scrape.
//!
//! `cauce_cache_entries` is a refreshed cell rather than a direct `Store`
...
use cauce_core::metrics::{render_prometheus, set_cache_entries};

/// Prometheus exposition content type for `GET /metrics`.
pub const METRICS_CONTENT_TYPE: &str = "text/plain; version=0.0.4; charset=utf-8";

/// Owns the store reference backing the `cauce_cache_entries` gauge. Cheap
/// to clone (one `Arc`); the record path needs no handle at all because
/// `cauce_core::metrics` is process-global.
#[derive(Clone)]
...
impl MetricsHandle {
    ...
    /// Refresh the `cauce_cache_entries` cell from the store. Called by the

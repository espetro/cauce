//! JSON-lines `tracing` layer writing `logs/cauce-YYYY-MM-DD.jsonl`.
//!
...
//! `cauce trace <id>` a filter over whole files.
//!
//! Filename: `tracing-appender` produces `cauce.<date>.jsonl`
//! (`{prefix}.{date}.{suffix}` is baked into its builder), so a thin
//! `Write` wrapper renames `cauce.<date>.jsonl` to `cauce-<date>.jsonl` ahead of
...
//! pruning still matches renamed files on the `cauce` prefix / `jsonl`
...
/// How often the writer checks for `cauce.<date>.jsonl` files to canonicalise.
...
pub fn daily_file_writer(
    ...
    Ok(
        tracing_appender::non_blocking::NonBlockingBuilder::default()
            .lossy(false)
            .thread_name("cauce-log-writer")
            .finish(DailyJsonlWriter::new(appender, dir)),
    )
}

/// Rename `cauce.<YYYY-MM-DD>.jsonl` files to `cauce-<YYYY-MM-DD>.jsonl`.
///
...
pub fn normalize_log_names(dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        let Some(rest) = name.strip_prefix("cauce.") else {
            continue;
        };
        ...
        let target = dir.join(format!("cauce-{date}.jsonl"));
        if !target.exists() {

...
//!   (`<data_dir>/logs/cauce-YYYY-MM-DD.jsonl`, daily rotation via
//!   `tracing-appender`, retention `retention_days`) and, when stderr is a
//!   TTY or `CAUCE_LOG_PRETTY=1`, a pretty stderr layer.
...
//! - `trace` reads the JSONL files for `cauce trace <request_id>`; W2's
//!   `/trace/{id}` page reuses the same reader.
//! - `tail` renders JSONL records as terminal lines for `cauce tail`
...
/// The cauce data directory. Delegates to `cauce_core::config::Dirs` so there
/// is exactly one resolver: `CAUCE_DATA_DIR` > `XDG_DATA_HOME` >
/// `~/.local/share/cauce`.
...
pub struct ObservabilityConfig {
    ...
    /// `CAUCE_LOG_RETENTION_DAYS` overrides until config lands.
    pub retention_days: usize,
    /// Emit the pretty human layer on stderr. Default: `CAUCE_LOG_PRETTY=1`
    /// or stderr is a TTY.
    pub stderr_pretty: bool,
    /// `tracing` filter directive for the stderr and OTLP layers. Default:
    /// `CAUCE_LOG`, then `RUST_LOG`, then `info`. The JSONL layer is NOT
    /// filtered by this: it keeps a fixed `info` floor so `RUST_LOG=warn`
    /// cannot silently starve `cauce trace`.
    pub filter: String,
...
impl Default for ObservabilityConfig {
    fn default() -> Self {
        let retention_days = std::env::var("CAUCE_LOG_RETENTION_DAYS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(7);
        let stderr_pretty = std::env::var_os("CAUCE_LOG_PRETTY").is_some_and(|v| v == "1")
            || std::io::stderr().is_terminal();
        let filter = std::env::var("CAUCE_LOG")
            .ok()
...
pub fn build(
    ...
    let filter = EnvFilter::try_new(&config.filter).unwrap_or_else(|e| {
        eprintln!(
            "cauce: invalid log filter {:?} ({e}); falling back to \"info\"",
            config.filter
        );
        EnvFilter::new("info")
    });
    ...
    // The JSONL layer is the observability record of truth and `cauce trace`'s
    // only input: it gets its own fixed `info` floor so a restrictive
    // `CAUCE_LOG`/`RUST_LOG` (e.g. `warn`) cannot silently empty the logs.
...
/// Called once by `cauce serve` (and `cauce mcp`); must run before any spans are

//! `cauce tail` support: turn JSONL log records into pretty terminal lines.
//!
...
//! reported at WARN, so `cauce tail --level warn` still surfaces engine
//! failures even though engine spans are declared at INFO.
//!
//! The file side (listing log files, parsing records) is shared with
//! `cauce trace` in `super::trace`; this module only filters and renders.
//!
...
/// Display filters for [`Tail`]; populated from `cauce tail` flags.

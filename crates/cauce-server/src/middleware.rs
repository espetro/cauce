...
//! and `cauce trace <id>` can rebuild it. An inbound `X-Request-Id` header is
...
use cauce_core::ClientKind;
use cauce_core::config::{host_part, is_loopback_host};
...
impl RequestCtx {
    ...
    pub fn store(&self, e: &cauce_core::StoreError) -> ApiError {
...
/// `X-Cauce-Client` header wins (`ui`, `api`, `cli`, `mcp` / `mcp:<name>`),

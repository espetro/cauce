...
//! before `cauce-server` starts serving still lands its samples. There is no
//! external metrics SDK in this crate; OTLP export of traces stays opt-in
//! behind `cauce-server`'s non-default `otlp` cargo feature.
//!
...
/// Bucket bounds for `cauce_engine_results` (result counts per call).
const COUNT_BUCKETS: &[f64] = &[0.0, 1.0, 2.0, 5.0, 10.0, 25.0, 50.0, 100.0];

/// `phase` label domain of `cauce_engine_duration_ms`. Engines record the
...
/// `outcome` label value for `cauce_engine_requests_total`: `"ok"`, or the
...
struct RegistryInner {
    ...
    /// `cauce_ttfr_ms`.
    ttfr: Hist,
    /// `cauce_engine_requests_total{engine,outcome}`.
    engine_requests: BTreeMap<Labels, u64>,
    /// `cauce_engine_duration_ms{engine,phase}`.
    engine_duration: BTreeMap<Labels, Hist>,
    /// `cauce_engine_results{engine}`.
    engine_results: BTreeMap<Labels, Hist>,
    /// `cauce_engine_breaker_state{engine}` — last write wins (gauge).
...
/// Backing cell of the `cauce_cache_entries` gauge. `cauce-server` refreshes it
...
/// `cauce_cache_entries`. Called by `cauce-server`'s `MetricsHandle`.
...
/// Label-free histogram (`cauce_ttfr_ms`, `cauce_admission_wait_ms`).
...
pub fn render_prometheus() -> String {
    ...
    let _ = writeln!(
        out,
        "# HELP cauce_cache_entries Live (unexpired) cache_entries rows"
    );
    let _ = writeln!(out, "# TYPE cauce_cache_entries gauge");
    let _ = writeln!(
        out,
        "cauce_cache_entries {}",
...
impl Metrics {
    ...
    /// `cauce_engine_results{engine}` when the call produced an answer
    ...
    /// `cauce_engine_duration_ms{engine,phase}` — recorded by engine runtimes
    ...
    /// `cauce_engine_breaker_state{engine}` — current breaker state as
    ...
    /// `cauce_admission_wait_ms` — time the flight leader spent in
    ...
    /// `cauce_admission_rejected_total{reason}` — one per request that got a
    ...
    /// `cauce_deadline_hit_total` — one per flight whose hard deadline

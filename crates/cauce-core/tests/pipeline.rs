...
//!
//! Integration test rather than a unit module deliberately: `cauce-engines`
//! is a cyclic dev-dependency, and only an integration test links the same
//! `cauce-core` build `Replay` implements `Engine` against.
//!
...
/// flight still hit the deadline, so `cauce_deadline_hit_total` must count
...
async fn all_engines_timeout_still_counts_deadline_hit() {
    ...
    let before = cauce_core::metrics::admission_stats().deadline_hits;
    ...
    assert_eq!(
        cauce_core::metrics::admission_stats().deadline_hits,
        before + 1,

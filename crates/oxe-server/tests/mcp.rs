...
//!
//! An rmcp streamable-HTTP client talks to a real `oxe serve` router (same
...
use oxe_core::{AuditFilter, ClientKind, HistoryFilter, HistoryItem, SearchPipeline, StoreTuning};
...
const CLIENT_NAME: &str = "oxe-mcp-http-test";
...
async fn mcp_http_exa_search_frozen_shape() {
    ...
    state
        .store()
        .record_click(oxe_core::ClickRow {
            id: None,
            ts: chrono::Utc::now(),
            query_hash: None,

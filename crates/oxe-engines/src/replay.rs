...
//!    `empty`), configured via `ReplayOpts` or the `OXE_REPLAY_*` env vars.
//! 2. Cassette mode (page 1 only): `<fixtures_root>/<engine>/<sha8>.json`
//!    written by `oxe record` (see [`crate::record`]). With
...
impl Replay {
    ...
    /// Build from the `OXE_REPLAY_*` environment:
    /// `OXE_REPLAY_LATENCY_MS`, `OXE_REPLAY_FAIL_EVERY`, `OXE_REPLAY_BLOCKED`,
    /// `OXE_REPLAY_EMPTY`, `OXE_REPLAY_PAGE_LIMIT`, plus the W0-local
    /// `OXE_REPLAY_FIXTURES_DIR` and `OXE_REPLAY_CASSETTE_ENGINE`.
    pub fn from_env() -> Self {
        let mut opts = ReplayOpts::default();
        if let Some(dir) = env_opt("OXE_REPLAY_FIXTURES_DIR") {
            opts.fixtures_root = PathBuf::from(dir);
        }
        if let Some(engine) = env_opt("OXE_REPLAY_CASSETTE_ENGINE") {
            opts.cassette_engine = Some(EngineId::from(engine));
        }
        if let Some(v) = env_opt("OXE_REPLAY_LATENCY_MS").and_then(|v| v.parse().ok()) {
            opts.latency_ms = v;
        }
        if let Some(v) = env_opt("OXE_REPLAY_FAIL_EVERY").and_then(|v| v.parse().ok()) {
            opts.fail_every = v;
        }
        if let Some(v) = env_opt("OXE_REPLAY_BLOCKED") {
            opts.blocked = env_truthy(&v);
        }
        if let Some(v) = env_opt("OXE_REPLAY_EMPTY") {
            opts.empty = env_truthy(&v);
        }
        if let Some(v) = env_opt("OXE_REPLAY_PAGE_LIMIT").and_then(|v| v.parse().ok()) {
...
mod synth {
    ...
    use oxe_core::{EngineId, SearchResult, normalize_url};

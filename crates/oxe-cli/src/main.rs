//! `oxe` binary: `serve`, `record`, `trace`, `tail`, `config` are
//! implemented; `search`, `engine` and `cache` land in later waves.
//!
...
fn main() {
    ...
    match sub.as_str() {
        ...
        _ => {
            eprintln!("usage: oxe <serve|record|trace|tail|config> [args]");
            std::process::exit(2);
        }
    }
}

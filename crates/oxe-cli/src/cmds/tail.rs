//! `oxe tail [--follow] [--request <id>] [--level warn] [--engine bing]
//! [-n LINES]`: pretty-print the JSONL logs, one line per event, spans
//! collapsed (`engine=bing 640ms ok`).
//!
//! Reads the newest `oxe-YYYY-MM-DD.jsonl` under `logs/` (last `-n` lines,
...
//! `oxe_server::observability::tail`; colour is on when stdout is a TTY and
//! `NO_COLOR` is unset.
//!
...
use oxe_server::observability::tail::{self, Tail, TailFilter};
use oxe_server::observability::trace::{self, LogRecord};

const USAGE: &str = "usage: oxe tail [--follow] [--request <id>] [--level <lvl>] \
                     [--engine <id>] [-n <lines>]";
...
pub fn run(args: &[String]) -> i32 {
    ...
    let files = match trace::log_files(&dir) {
        ...
        _ => {
            eprintln!(
                "oxe tail: no JSONL logs under {} (is `oxe serve` running?)",
                dir.display()
            );
            return 1;
        }
    ...
    let mut offset = match initial_read(&path, opts.lines, &mut tail) {
        Ok(offset) => offset,
        Err(e) => {
            eprintln!("oxe tail: cannot read {}: {e}", path.display());
            return 1;
        }
    };
    ...
    loop {
        ...
        match drain(&path, &mut offset, &mut tail, &mut out) {
            ...
            Ok(_) => {}
            Err(e) => {
                eprintln!("oxe tail: {}: {e}", path.display());
                return 1;
            }
        }
        let _ = out.flush();

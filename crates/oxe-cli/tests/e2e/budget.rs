...
//! RSS for each mode: `oxe serve` after 100 replay searches (< 80 MB),
//! `oxe serve --headless` (< 50 MB) and `oxe mcp` stdio (< 40 MB).
//! Only runs when `OXE_NIGHTLY=1`.
//!
...
/// Build the release binary (shared `OXE_BUDGET_TARGET_DIR`, so repeat
...
/// W1-12 mode budgets: `oxe serve --headless` idles under 50 MB RSS and
/// `oxe mcp` (stdio, no listener) under 40 MB.
#[tokio::test]
async fn release_headless_and_mcp_rss() {
    ...
    // `oxe serve --headless`: API surface only, warmed by a few searches.
    let server = common::spawn_oxe_bin_full(
        bin.to_str().unwrap(),
    ...
    // `oxe mcp`: stdio transport, no listener. stdin stays piped open so
    // the transport idles instead of seeing EOF and exiting.
    let mut child = Command::new(&bin)
        .arg("mcp")
        .current_dir(common::workspace_root())
        .env("OXE_DATA_DIR", tmp.path().join("mcp-data"))
        .env("OXE_CONFIG_DIR", tmp.path().join("mcp-cfg"))
    ...
    let pid = child.id().expect("oxe mcp pid");
    let rss = rss_kb(pid).await;
    let _ = child.kill().await;
    let _ = child.wait().await;
    assert!(
        rss < FORTY_MB_KB,
        "oxe mcp RSS is {rss} KB, >= {FORTY_MB_KB} KB"
    );
}

...
//!
//! Runs the `oxe` binary, starts `oxe serve` with the replay engine, and
//! walks through the full user flow: network search, cache hit, history,
//! stats, click beacon, HTML result page, cache delete, audit, and `oxe trace`.
//!
...
async fn replay_golden_path() {
    ...
    let server = common::spawn_oxe(&data_dir, &config_dir, "replay").await;
...
async fn live_ddgs_first_two_assertions() {
    ...
    let server = common::spawn_oxe(&data_dir, &config_dir, "ddgs").await;
    let addr = server.addr;

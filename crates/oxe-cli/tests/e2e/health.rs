...
const BLOCKED: &[(&str, &str)] = &[("OXE_REPLAY_BLOCKED", "1")];

/// Start `oxe serve` with a blocked replay engine on the given data dir.
async fn spawn_blocked(data_dir: &TempDir, config_dir: &TempDir) -> common::ServerGuard {
    common::spawn_oxe_env(data_dir.path(), config_dir.path(), "replay", BLOCKED).await
}

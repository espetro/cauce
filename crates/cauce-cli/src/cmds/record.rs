//! `cauce record --engine <id> --query <q>`: run an engine once and write a
...
use cauce_core::Engine;
use cauce_core::config::Config;
use cauce_engines::factory::build_engine;
use cauce_engines::record;

const USAGE: &str = "usage: cauce record --engine <id> --query <q> [--out-dir <engines/fixtures>]";
...
/// `Config::load()` picks up `[[engines]]` entries and `CAUCE_ENGINES`, and

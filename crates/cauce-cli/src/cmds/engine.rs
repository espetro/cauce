...
use cauce_core::{ClientKind, Engine, SafeSearch, SearchRequest};
...
const USAGE: &str = "usage: cauce engine test <spec.yaml> [--live \"<query>\"] [--record] [--fixtures-dir <engines/fixtures>]";
...
fn dispatch(opts: EngineArgs) -> Result<i32, String> {
    let spec = compile_spec_source(
        &opts.spec,
        &cauce_core::config::Dirs::detect().config_dir,
        &system_env(),
    )

//! `cauce serve [--bind ADDR] [--port N] [--headless]`: run the HTTP server.
//!
...
const USAGE: &str = "usage: cauce serve [--bind ADDR] [--port N] [--headless]";
...
pub fn run(args: &[String]) -> i32 {
    ...
    let guard = match observability::init(&obs) {
        Ok(guard) => guard,
        Err(e) => {
            eprintln!("cauce serve: logging init failed: {e}");
            return 1;
        }
    };
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("cauce serve: tokio runtime: {e}");
            return 1;
        }
    };
...
async fn serve_async(opts: ServeOpts, cfg: Config, host: String) -> i32 {
    ...
    match listener.local_addr() {
        Ok(addr) => tracing::info!(%addr, headless = opts.headless, "cauce serve listening"),

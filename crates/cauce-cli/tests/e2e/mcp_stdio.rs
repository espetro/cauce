//! `cauce mcp` stdio e2e (W1-08 acceptance 3).
//!
...
// The `cauce mcp` subcommand exists only in `mcp` builds (W1-12).
#![cfg(feature = "mcp")]
...
async fn mcp_stdio_search() {
    ...
    let mut child = Command::new(common::cauce_bin())
        .current_dir(common::workspace_root())
        .arg("mcp")
        .env("CAUCE_DATA_DIR", data_dir.as_os_str())
        .env("CAUCE_CONFIG_DIR", config_dir.as_os_str())
    ...
    let client: RunningService<RoleClient, InitializeRequestParams> =
        timeout(Duration::from_secs(30), async {
            InitializeRequestParams::new(
                ...
                Implementation::new("cauce-mcp-stdio-test", "0.0.0"),
            )
            .serve((stdout, stdin))
            .await
        })
    ...
    let status = timeout(Duration::from_secs(10), child.wait())
        ...
        .expect("child wait");
    assert!(status.success(), "cauce mcp exit status: {status}");
    ...
    assert!(
        ...
        "cauce mcp must not bind a listener; stderr:\n{joined}"
    );
}

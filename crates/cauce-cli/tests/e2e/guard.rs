//! W1-13 e2e: `cauce serve` refuses a non-loopback bind while admin auth is
...
use crate::common::{cauce_bin, spawn_cauce, workspace_root};

/// `cauce serve --bind 0.0.0.0` exits non-zero with a clear refusal: auth is
...
async fn non_loopback_bind_is_refused() {
    ...
    let output = timeout(
        ...
        Command::new(cauce_bin())
            ...
            .env("CAUCE_CONFIG_DIR", tmp.path().join("cfg"))
            .env("CAUCE_ENGINES", "replay")
            .output(),
    )
    .await
    .expect("refusal must not hang")
    .expect("spawn cauce serve");
...
async fn foreign_host_and_origin_get_403() {
    let tmp = tempfile::tempdir().unwrap();
    let server = spawn_cauce(tmp.path().join("data"), tmp.path().join("cfg"), "replay").await;
